...
// Structure to hold one epoch of position data with MEMS zero bias
struct PosEpoch {
    ...
    // MEMS zero bias data
    double bgx;              // Gyro bias X (deg/h)
    double bgy;              // Gyro bias Y (deg/h)
    double bgz;              // Gyro bias Z (deg/h)
    double bax;              // Accel bias X (ug)
    double bay;              // Accel bias Y (ug)
    double baz;              // Accel bias Z (ug)
};
...
public:
    ...
    // Get time series for plotting zero bias data
    void getTimeSeries(QVector<double> &time,
